...
def bilinear_sampler(img, coords, mode='bilinear', mask=False):
    ...
    grid = torch.cat([xgrid, ygrid], dim=-1).to(img.device)
...
def bilinear_sampler_cre(img, coords, mode='bilinear', mask=False):
    """ Wrapper for grid_sample, uses pixel coordinates """
    H, W = img.shape[-2:]
    xgrid, ygrid = coords.split([1, 1], dim=-1)
    xgrid = 2 * xgrid / (W - 1) - 1
...
def disp_error_img(D_est_tensor, D_gt_tensor, abs_thres=3., rel_thres=0.05, dilate_radius=1):
    '''
    D_est_tensor need to be <0
    D_gt_tensor need to be <0
    '''
