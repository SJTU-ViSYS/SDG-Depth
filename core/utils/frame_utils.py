...
def readPFM_ffr(fp):
    ...
    header = fp_BytesIO.readline().rstrip()
    if header == b'PF':
        color = True
    elif header == b'Pf':
        color = False
    ...
    dim_match = re.match(rb'^(\d+)\s(\d+)\s$', fp_BytesIO.readline())
    if dim_match:
        width, height = map(int, dim_match.groups())
    else:
        raise Exception('Malformed PFM header.')

    scale = float(fp_BytesIO.readline().rstrip())
    # if scale < 0:  # little-endian
    #     endian = '<'
    #     scale = -scale
    # else:
    ...
    data = np.frombuffer(fp[18:], dtype=np.float32)
    shape = (height, width, 3) if color else (height, width)
...
def read_gen_ffr(fp, file_name):
    ...
    if ext == '.png' or ext == '.jpeg' or ext == '.ppm' or ext == '.jpg':
        ...
        return readFlow(fp).astype(np.float32)
    elif ext == '.pfm':
        flow = readPFM_ffr(fp).astype(np.float32)
        if len(flow.shape) == 2:
            return flow
        else:
            return flow[:, :, :-1]
