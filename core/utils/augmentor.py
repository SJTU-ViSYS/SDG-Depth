...
class FlowAugmentor:
    def __init__(self, args, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=True, yjitter=False,
        ...
        self.occlusion_aug_prob = args.occlusion_aug_prob
    ...
    def spatial_transform(self, img1, img2, flow, hint=None, dxy=None):
        ...
        if self.yjitter:
            ...
            if hint is not None:
                hint = hint[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]

            if dxy is not None:
                dxy = dxy[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        ...
        # add random occlusion to right image
        if np.random.rand() < self.occlusion_aug_prob:
            occ_w = np.random.randint(50, 100)
            occ_h = np.random.randint(50, 100)
            h, w = self.crop_size[0], self.crop_size[1]

            dst_w = np.random.randint(0, w - occ_w + 1)
            dst_h = np.random.randint(0, h - occ_h + 1)

            img2[dst_h:dst_h + occ_h, dst_w:dst_w + occ_w, :] = np.mean(np.mean(img2, 0), 0)[np.newaxis, np.newaxis]


        return img1, img2, flow, hint, dxy

    def __call__(self, img1, img2, flow, hint=None, dxy=None):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)


        img1, img2, flow, hint, dxy = self.spatial_transform(img1, img2, flow, hint, dxy)

        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        flow = np.ascontiguousarray(flow)
        if hint is not None:
            hint = np.ascontiguousarray(hint)
        if dxy is not None:
            dxy = np.ascontiguousarray(dxy)

        if hint is not None:
            return img1, img2, flow, hint
        if dxy is not None:
            return img1, img2, flow, dxy
        return img1, img2, flow


class SparseFlowAugmentor:
    def __init__(self, args, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=False, yjitter=False,
    ...
    def spatial_transform(self, img1, img2, flow, valid, hint=None, more_bottom=0.0):
        # randomly sample scale
        ...
        if more_bottom:
            ...
            y_start = self.args.y_start

            y0 = np.random.randint(y_start, img1.shape[0] - self.crop_size[0] + margin_y)
        ...
        return img1, img2, flow, valid, hint

    def __call__(self, img1, img2, flow, valid, hint=None, more_bottom=0.0):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow, valid, hint = self.spatial_transform(img1, img2, flow, valid, hint=hint,
